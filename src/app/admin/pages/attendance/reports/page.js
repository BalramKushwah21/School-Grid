"use client"; // Next.js me hooks use karne ke liye zaroori hai
import React, { useState } from "react";

export default function ReportDashboard() {
	// 1. Table ka data state mein store karein (Future me yeh API se aayega)
	...
	// 2. Search Query ke liye state
	const [searchQuery, setSearchQuery] = useState("");

	// 3. Search input ke basis par table data filter karein
		...
		<div className="min-h-screen bg-slate-50 p-8 font-sans">
			...
			<div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					...
					<p className="text-slate-500 text-sm font-medium">
						Monitoring real-time registry compliance and status
						logs.
					</p>
				</div>
			...
			<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
				<div className="p-6 border-b border-slate-100 flex items-center justify-between">
					...
					{/* SEARCH INPUT UPDATE */}
					<input
						className="bg-slate-50 border border-slate-200 px-4 py-2 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
